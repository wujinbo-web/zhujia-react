...
import style from './configureBox.scss';
    ...
    render() {
        return (
            <div className={style['con']}>
                {this.props.datas.length <= 6 ? this.props.datas.map(this.getListItem) : this.props.datas.slice(0, 5).map(this.getListItem)}
                {this.props.datas.length <= 6 ? <div/> : <div className={style['ddd']} onTouchEnd={(e) => this.openBox(e)}><img src="/static/tenant/icon/ddd.png" /></div>}
            </div>
        );  
    }
}
